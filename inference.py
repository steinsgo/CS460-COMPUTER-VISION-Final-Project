...
# Make imports portable across machines (Windows/Linux) without hard-coded paths.
_repo_root = Path(__file__).resolve().parent
sys.path.append(str(_repo_root))
sys.path.append(str(_repo_root / 'basicsr'))
import numpy as np
...
# Optional deps (not required for saving restored images).
try:
...
def test_tile(model, input, tile_size=512, tile_pad=16):
    ...
    # Weighted overlap-add to reduce visible seams between tiles.
    output = input.new_zeros((batch, channel, height, width))
    weight_sum = input.new_zeros((batch, 1, height, width))
    ...
    def _make_1d_weight(length: int, pad: int, left_edge: bool, right_edge: bool, ref_tensor: torch.Tensor):
        if length <= 1:
            return ref_tensor.new_ones((length,))
        pad = int(max(0, min(pad, length // 2)))
        w = ref_tensor.new_ones((length,))
        if pad > 0 and not left_edge:
            ramp = torch.linspace(0.0, 1.0, steps=pad + 1, device=ref_tensor.device, dtype=ref_tensor.dtype)[1:]
            w[:pad] = ramp
        if pad > 0 and not right_edge:
            ramp = torch.linspace(1.0, 0.0, steps=pad + 1, device=ref_tensor.device, dtype=ref_tensor.dtype)[:-1]
            w[-pad:] = ramp
        return w

    def _make_2d_weight(hh: int, ww: int, pad: int, top_edge: bool, bottom_edge: bool, left_edge: bool, right_edge: bool, ref_tensor: torch.Tensor):
        wy = _make_1d_weight(hh, pad, top_edge, bottom_edge, ref_tensor)
        wx = _make_1d_weight(ww, pad, left_edge, right_edge, ref_tensor)
        w2d = (wy[:, None] * wx[None, :]).unsqueeze(0).unsqueeze(0)
        return w2d

    tiles_x = math.ceil(max(width - tile_pad * 2, 1) / stride)
    tiles_y = math.ceil(max(height - tile_pad * 2, 1) / stride)
    ...
    for y in range(tiles_y):
        for x in range(tiles_x):
            ...
            input_start_x = min(ofs_x, max(width - tile_size, 0))
            input_start_y = min(ofs_y, max(height - tile_size, 0))
            ...
            input_tile = check_image_size(input_tile, window_size=128)

            output_tile = model(input_tile)
            if isinstance(output_tile, (list, tuple)):
                output_tile = output_tile[0]
            output_tile = output_tile[:, :, :tile_h, :tile_w]

            core_h = input_end_y - input_start_y
            core_w = input_end_x - input_start_x
            core_start_y = input_start_y - input_start_y_pad
            core_start_x = input_start_x - input_start_x_pad
            core_end_y = core_start_y + core_h
            core_end_x = core_start_x + core_w
            output_core = output_tile[:, :, core_start_y:core_end_y, core_start_x:core_end_x]

            w2d = _make_2d_weight(
                core_h,
                core_w,
                tile_pad,
                top_edge=(input_start_y == 0),
                bottom_edge=(input_end_y == height),
                left_edge=(input_start_x == 0),
                right_edge=(input_end_x == width),
                ref_tensor=input,
            )

            output[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += output_core * w2d
            weight_sum[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += w2d
...
def main():
    ...
    #                     help='groundtruth image')
    parser.add_argument('-i', '--input', type=str,
                        default='/data/liuyidi/nitre_2023_dehaze/data_dehaze/UHD_noise/test',
                        help='Input image or folder')
    parser.add_argument('-g', '--gt', type=str,
    ...
    parser.add_argument('-w', '--weight', type=str,
                        ...
                        help='path for model weights')
    parser.add_argument('-o', '--output', type=str, default='/data/liuyidi/nitre_2023_dehaze/data_dehaze/UHD_noise/test_noise_50', help='Output folder')
    ...
    parser.add_argument('--add_gaussian', type=int,default=0, help='Add Gaussian noise')
    
    parser.add_argument('--test_tile', action='store_true', help='Test with tile')
    parser.add_argument('--tile_size', type=int, default=1280, help='Tile size for --test_tile')
    parser.add_argument('--tile_pad', type=int, default=32, help='Overlap padding (pixels) for --test_tile')
    ...
    for idx, path in enumerate(paths):
        ...
        if args.add_gaussian:
            ...
            img, _ = _add_gaussian_noise(img, args.add_gaussian)
        ...
        if args.test_tile:
            with torch.no_grad():
                import time
                t0 = time.time()
                output = test_tile(EnhanceNet, img_tensor, tile_size=args.tile_size, tile_pad=args.tile_pad)
                # output = test_tile(EnhanceNet, img_tensor,128)
                t1 = time.time()
                print('time:', t1-t0)
        else:
            with torch.no_grad():
