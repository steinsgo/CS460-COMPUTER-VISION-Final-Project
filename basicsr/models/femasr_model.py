...
class FeMaSRModel(BaseModel):
    def __init__(self, opt):
        super().__init__(opt)
        self.test_tile = opt['val'].get('test_tile', False)
    ...
    def optimize_parameters(self, current_iter,stage=None):
        train_opt = self.opt['train']
        ...
        if self.filter:
            ...
            self.output = self.net_g(self.lq,self.gt,stage)
        else:
    ...
    def test(self):
        ...
        if h * w > min_size or self.test_tile:
            self.output = net_g.test_tile(lq_input,tile_size= self.test_tile)
        else:
