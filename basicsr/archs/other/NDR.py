...
class ResidualGroup(nn.Module):
    def __init__(self, conv=default_conv, n_feat=64, kernel_size=3, reduction=16, act=nn.ReLU(True), res_scale=1, n_resblocks=1):
...
class NDRN(nn.Module):
    ...
    def forward(self, LQ_img,HQ_img):
        HQ_img = self.restoration_process(LQ_img)
        LQ_img = self.degradation_process(HQ_img)
...
if __name__ == "__main__":
    ...
    name = "our"
    total_ops, total_params = profile(model, (HF,LF))
    print(
        "%s         | %.4f(M)      | %.4f(G)         |"
