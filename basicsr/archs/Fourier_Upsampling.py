...
def get_D_map_optimized(feature):
    ...
    i_indices = torch.arange(H, dtype=torch.float32).reshape(1, 1, H, 1).contiguous().repeat(1, 1, 1, W).cuda()
    j_indices = torch.arange(W, dtype=torch.float32).reshape(1, 1, 1, W).contiguous().repeat(1, 1, H, 1).cuda()
