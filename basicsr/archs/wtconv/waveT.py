...
from basicsr.archs.wtconv.DWT_IDWT_Functions import DWTFunction_1D, IDWTFunction_1D, \
...
if __name__ == '__main__':
    dwt = DWT_2D("haar")
    iwt = IDWT_2D("haar")
    x = np.array(Image.open(os.path.join(self.opt['bulr_dir'], 'input/',  str(num)+'_'+sample["clean_id"])))
    x = torch.rand(3, 3, 24, 24).cuda()
    xll = x
    wavelet_list = []
    for i in range(3):
