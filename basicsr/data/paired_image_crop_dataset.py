...
class PairedcropImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(PairedcropImageDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        file_name = str(num)+'_'+ path.split('/')[-1]
        #返回path上两级目录
        path = os.path.dirname(os.path.dirname(path))
        gt_path = os.path.join(path, 'gt', file_name)
        ...
        lq_path = os.path.join(path, 'input', file_name)
